...
const close = Joi.object()
  .keys({ gameId: id, gameCode: code })
  .or("gameId", "gameCode");
const gDelete = Joi.object()
  .keys({
    id,
    code,
  })
  .or("id", "code");
const playerAdd = Joi.object()
  .keys({ gameId: id, gameCode: code })
  .or("gameId", "gameCode");
const playerSet = Joi.object()
  .keys({
    gameId: id,
    gameCode: code,
    ready: Joi.boolean(),
    nextGame: Joi.boolean(),
  })
  .or("gameId", "gameCode");
const playerSetOrder = Joi.object()
  .keys({
    gameId: id,
    gameCode: code,
    playerList: Joi.array().required(),
  })
  .or("gameId", "gameCode");
const playerRemove = Joi.object()
  .keys({ gameId: id, gameCode: code, userId: id })
  .or("gameId", "gameCode");
const startGame = Joi.object()
  .keys({ gameId: id, gameCode: code })
  .or("gameId", "gameCode");
const processAction = Joi.object()
  .keys({
    gameId: id,
    gameCode: code,
